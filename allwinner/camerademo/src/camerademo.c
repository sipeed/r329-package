...
static int capture_photo(camera_handle *camera)
{
	...
	while (np < camera->photo_num) {
		...
		if (isOutputRawData(camera->pixelformat)) {
			sprintf(source_data_path, "%s/%s%s_%d%s", camera->save_path, "source_", get_format_name(camera->pixelformat), np + 1, ".raw");
			...
			if (((camera->photo_type == ALL_TYPE) || (camera->photo_type == BMP_TYPE)) && (camera->ToRGB24 != NULL)) {
				...
				if (support_tinatest == 0x55) {
					...
					sprintf(bmp_data_path, "%s/%s%d_%s_%d%s", camera->save_path, "bmp_camera", camera->camera_index, get_format_name(camera->pixelformat), (np + 1) % 2, ".bmp");
				} else if (support_tinatest == 0xff) {
					memset(bmp_data_path, 0, sizeof(bmp_data_path));
					sprintf(bmp_data_path, "%s/%s%d_%s_%d%s", camera->save_path, "bmp_reopen_camera", camera->camera_index, get_format_name(camera->pixelformat), np + 1, ".bmp");
				}
